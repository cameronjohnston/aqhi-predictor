...
def process_wind_velocity_data(
    ...
    print(f'Got {len(wind_velocities)} wind velocities from {type(data_source).__name__}')

    data_target.save(wind_velocities)
    print(f'Saved {len(wind_velocities)} wind velocities to {type(data_target).__name__}')
    ...
    if avg_data_target and velocity_service:
        ...
        print(f'Saved {len(averages)} wind velocity daily averages to {type(avg_data_target).__name__}')
